...
import {setWinner, setOpponentInfo} from '../../store/game';
...
function SinglePlayerChoose(props) {
  ...
  const readyButtonHandle = () => {
    ...
    setOpponentInfo({
      guestUsername: 'Team Rocket',
      guestPhotoUrl: 'pics/teamrocket.jpg',
    });
    fetchComputerPokemon().then(() => {
      setIsLoading(false);
      history.push('./spgame');
      if (musicOn) {
...
const mapDispatch = (dispatch) => {
  return {
    ...
    fetchComputerPokemon: () => dispatch(fetchComputerPokemon()),
    setWinner: (pk, user, opp) => dispatch(setWinner(pk, user, opp)),
    startMusic: (src) => dispatch(startBattleMusic(src)),
    setOpponentInfo: (info) => dispatch(setOpponentInfo(info)),
  };
};
export default connect(mapState, mapDispatch)(SinglePlayerChoose);
